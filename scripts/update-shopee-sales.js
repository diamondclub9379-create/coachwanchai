const fs = require('fs');
const path = require('path');

// 1. Parse RAW_DATA for hash -> slug mapping
const genScript = fs.readFileSync(path.join(__dirname, 'generate-all-products.js'), 'utf8');
const rawDataMatch = genScript.match(/const RAW_DATA = `\n([\s\S]*?)`.trim\(\)/);
if (!rawDataMatch) { console.error('Could not parse RAW_DATA'); process.exit(1); }

function slugify(name, index) {
  const englishParts = name.match(/[a-zA-Z][a-zA-Z0-9'.!?&+\-]*/g);
  let slug = '';
  if (englishParts && englishParts.length > 0) {
    slug = englishParts.join(' ').toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-')
      .replace(/-+/g, '-').replace(/^-|-$/g, '');
  }
  if (slug.length < 3) slug = `product-${index}`;
  if (slug.length > 80) slug = slug.substring(0, 80).replace(/-$/g, '');
  return slug;
}

const lines = rawDataMatch[1].trim().split('\n');
const hashToSlug = {};
const usedSlugs = new Set();

lines.forEach((line, i) => {
  const parts = line.split('|');
  if (parts.length < 2) return;
  const name = parts[0].trim();
  const hash = parts[1].trim();
  let slug = slugify(name, i);
  if (usedSlugs.has(slug)) {
    let n = 2;
    while (usedSlugs.has(`${slug}-${n}`)) n++;
    slug = `${slug}-${n}`;
  }
  usedSlugs.add(slug);
  hashToSlug[hash] = slug;
});

console.log(`Mapped ${Object.keys(hashToSlug).length} hashes to slugs`);

// 2. Read sales data from shopee-sales.txt (hash:sold per line)
const salesFile = path.join(__dirname, 'shopee-sales.txt');
if (!fs.existsSync(salesFile)) {
  console.error('shopee-sales.txt not found.');
  process.exit(1);
}

const salesRaw = fs.readFileSync(salesFile, 'utf8');
const hashToSales = {};
salesRaw.trim().split('\n').forEach(line => {
  const idx = line.lastIndexOf(':');
  if (idx === -1) return;
  const hash = line.substring(0, idx).trim();
  const sold = parseInt(line.substring(idx + 1).trim());
  if (hash && !isNaN(sold)) hashToSales[hash] = sold;
});

console.log(`Loaded ${Object.keys(hashToSales).length} sales entries`);

// 3. Build slug -> weight (lower weight = higher priority in Hugo)
// Sort by sales descending, assign weight 1, 2, 3...
const slugSales = [];
for (const [hash, sold] of Object.entries(hashToSales)) {
  const slug = hashToSlug[hash];
  if (slug) {
    slugSales.push({ slug, sold });
  }
}
slugSales.sort((a, b) => b.sold - a.sold);

const slugToWeight = {};
slugSales.forEach((item, i) => {
  slugToWeight[item.slug] = i + 1;
});

console.log(`Matched ${slugSales.length} to slugs`);
console.log(`Top 10 best sellers:`);
slugSales.slice(0, 10).forEach((item, i) => {
  console.log(`  ${i + 1}. ${item.slug} (${item.sold} sold)`);
});

// 4. Update .md files with weight field
const productsDir = path.join(__dirname, '..', 'content', 'products');
const mdFiles = fs.readdirSync(productsDir).filter(f => f.endsWith('.md'));
let updated = 0, noMatch = 0, alreadyCorrect = 0;

mdFiles.forEach(file => {
  const slug = file.replace('.md', '');
  const weight = slugToWeight[slug];
  if (!weight) { noMatch++; return; }

  const filePath = path.join(productsDir, file);
  const content = fs.readFileSync(filePath, 'utf8');

  // Check if weight already exists and is correct
  const weightMatch = content.match(/weight:\s*(\d+)/);
  if (weightMatch && parseInt(weightMatch[1]) === weight) { alreadyCorrect++; return; }

  let newContent;
  if (weightMatch) {
    // Replace existing weight
    newContent = content.replace(/weight:\s*\d+/, `weight: ${weight}`);
  } else {
    // Add weight after title line
    newContent = content.replace(/(title:\s*"[^"]*"\n)/, `$1weight: ${weight}\n`);
  }

  if (newContent && newContent !== content) {
    fs.writeFileSync(filePath, newContent, 'utf8');
    updated++;
    if (updated <= 10) console.log(`  ${file}: weight → ${weight}`);
  }
});

console.log(`\nResults:`);
console.log(`  Updated: ${updated}`);
console.log(`  Already correct: ${alreadyCorrect}`);
console.log(`  No match: ${noMatch}`);
